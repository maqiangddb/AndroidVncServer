...
class VncCanvas extends Canvas
  ...
  public VncCanvas(VncViewer v, int maxWidth_, int maxHeight_)
    ...
    inputEnabled = false;
      System.out.println("VncCanvas----viewOnly:"+viewer.options.viewOnly);
  ...
  public void setPixelFormat() throws IOException {
    ...
    savedColorNum = viewer.options.colorNum;
    setPixelFormatWithType(savedColorNum);
    updateFramebufferSize();
  }

    void setPixelFormatWithType(int type) throws IOException {

        switch (type) {
            case RfbProto.BlackWhite:
                bytesPixel = 1;
                rfb.writeSetPixelFormat(8, 3, false, true, 1, 1, 1, 0, 1, 2);//Black/White
                cm8 = new DirectColorModel(8, 1, (1 << 1), (1 << 2));
                break;
            case RfbProto.Tens:
                //System.out.print("---bytesPixel:"+bytesPixel+"\n");
                bytesPixel = 1;
                //System.out.print("----bytesPixel:"+bytesPixel+"\n");
                rfb.writeSetPixelFormat(8, 6, false, true, 3, 3, 3, 0, 2, 4);//Tens
                cm8 = new DirectColorModel(8, 3, (3 << 2), (3 << 4));
                break;
            case RfbProto.Hundreds:
                bytesPixel = 1;
                rfb.writeSetPixelFormat(8, 8, false, true, 7, 7, 3, 0, 3, 6);//Hundreds
                cm8 = new DirectColorModel(8, 7, (7 << 3), (3 << 6));
                break;
            case RfbProto.Thousends:
                bytesPixel = 2;
                rfb.writeSetPixelFormat(16, 16, false, true, 63, 31, 31, 11, 6, 0);//Thousends
                break;
            case RfbProto.Millions:
                bytesPixel = 4;
                rfb.writeSetPixelFormat(32, 24, false, true, 255, 255, 255, 16, 8, 0);//Mini
                break;
        }
    }

    int transformPixel(int pixel) {
        int result = 0;
        for (int i = 0; i < colorDepth; i++) {
            int item = pixel >> (colorDepth - i - 1);
            if (item == 1) {
  ...
  public void processNormalProtocol() throws Exception {
	...
	*/
	...
	if (false) {//viewer.options.eightBitColors != (bytesPixel == 1)) {
          // Pixel format should be changed.
          setPixelFormat();
          fullUpdateNeeded = true;
	}

     if (viewer.options.colorNum != savedColorNum) {
         savedColorNum = viewer.options.colorNum;
         switch (savedColorNum) {
             case RfbProto.BlackWhite:
                 zrlePixFactor = 8;
                 colorDepth = 3;
                 break;
        ...
        rfb.writeFramebufferUpdateRequest(0, 0, w, h, false);//MQ solve refresh problem [!fullUpdateNeeded);]
  ...
  void handleHextileSubrect(int tx, int ty, int tw, int th)
    ...
    private void SeeData(byte[] data, int length) {
        if (length > 40) {
            System.out.println("--------------------------------------------");
            for (int i=0; i<20; i++) {
    ...
    private void debug_zrle(String msg) {
        if (true) {
  ...
  void handleZRLERect(int x, int y, int w, int h) throws Exception {
    ...
    debug_zrle("<<<<<<<handleZRLERect--statNumRectsZRLE:"+statNumRectsZRLE);
    ...
    //System.out.println("zrleBufLen:"+zrleBufLen);
    ...
    //System.out.println("MQ-------------nBytes:"+nBytes);
    //SeeData(zrleBuf, nBytes);

    zrleInStream.setUnderlying(new MemInStream(zrleBuf, 0, nBytes), nBytes);

    //zrleInStream.check(20);
    //DebugBuf(zrleInStream.getbuf());
    //SeeData(zrleInStream.getbuf(), zrleInStream.getend());
    //System.out.println("MQ--ptr:"+zrleInStream.getptr()+"--end:"+zrleInStream.getend());
