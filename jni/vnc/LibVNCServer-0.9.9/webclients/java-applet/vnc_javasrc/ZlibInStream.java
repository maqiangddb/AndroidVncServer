...
public class ZlibInStream extends InStream {
  ...
  protected int overrun(int itemSize, int nItems) throws Exception {
      System.out.println("<<<<<<<<<<overrun:"+itemSize+","+nItems);
    if (itemSize > bufSize)
  ...
  private void decompress() throws Exception {
    ...
    public void decompressAll() throws DataFormatException {
        if (inflater.needsInput()) {
            inflater.setInput(underlying.getbuf());
        }
        byte[] data;
        //int n = inflater.inflate(data);
    }
